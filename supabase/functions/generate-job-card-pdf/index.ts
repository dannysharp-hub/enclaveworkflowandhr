...
function generateSimplePdf(job: any, customer: any): Uint8Array {
  ...
  // Build a minimal PDF 1.4 using WinAnsiEncoding for proper £ and dash rendering
  const escapePdfBytes = (s: string): Uint8Array => {
    const escaped = s.replace(/\\/g, "\\\\").replace(/\(/g, "\\(").replace(/\)/g, "\\)");
    return pdfEncode(escaped);
  };
  ...
  // Build stream as binary (WinAnsi) content
  const streamParts: Uint8Array[] = [];
  const te = new TextEncoder();
  streamParts.push(te.encode("BT\n/F1 11 Tf\n"));

  for (const line of lines) {
    if (y < margin) break;
    streamParts.push(te.encode(`1 0 0 1 ${margin} ${y} Tm\n(`));
    streamParts.push(escapePdfBytes(line));
    streamParts.push(te.encode(") Tj\n"));
    y -= lineHeight;
  }
  streamParts.push(te.encode("ET\n"));

  // Concatenate stream
  const streamLen = streamParts.reduce((s, a) => s + a.length, 0);
  const streamBytes = new Uint8Array(streamLen);
  let pos = 0;
  for (const part of streamParts) { streamBytes.set(part, pos); pos += part.length; }

  // Build PDF structure (ASCII parts use TextEncoder, stream is binary)
  const pdfParts: Uint8Array[] = [];
  const offsets: number[] = [];
  let currentOffset = 0;

  const addAscii = (s: string) => { const b = te.encode(s); pdfParts.push(b); currentOffset += b.length; };
  const addObj = (s: string) => { offsets.push(currentOffset); addAscii(s); };
  ...
  // Object 4: Content stream (binary)
  offsets.push(currentOffset);
  addAscii(`4 0 obj\n<< /Length ${streamLen} >>\nstream\n`);
  pdfParts.push(streamBytes); currentOffset += streamBytes.length;
  addAscii("endstream\nendobj\n");
  ...
  for (const off of offsets) {
    addAscii(`${String(off).padStart(10, "0")} 00000 n \n`);
  }
  addAscii(`trailer\n<< /Size ${offsets.length + 1} /Root 1 0 R >>\nstartxref\n${xrefOffset}\n%%EOF\n`);

  const totalLen = pdfParts.reduce((s, a) => s + a.length, 0);
  const result = new Uint8Array(totalLen);
  let rpos = 0;
  for (const part of pdfParts) { result.set(part, rpos); rpos += part.length; }
  return result;
}
